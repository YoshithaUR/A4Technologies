...
class SoftwareCompanyMouseEffect {
    ...
    init() {
        ...
        console.log('🖱️ Software Company Mouse Effect Activated');
    }
    ...
    onHover(config, element) {
        ...
        // Pulse effect
        if (config.pulse) {
            this.elements.follower.style.animation = 'mousePulse 1.5s ease-in-out infinite';
        }
    ...
    startAnimation() {
        const animate = () => {
            ...
            this.follower.x += (this.mouse.x - this.follower.x) * this.settings.followSpeed;
            this.follower.y += (this.mouse.y - this.follower.y) * this.settings.followSpeed;
    ...
    setupCompanySpecificEffects() {
        ...
        const techItems = document.querySelectorAll('.tech-item, .skill, .technology');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (body.classList.contains('tech-startup') || body.dataset.theme === 'startup') {
        mouseEffect = initTechStartupMouse();
    } else if (body.classList.contains('corporate') || body.dataset.theme === 'corporate') {
